...
7. Have a second, already-clean Word file with the styles you want to standardize on? Use **Upload your own** (New Styles panel footer) to import its style definitions as additional merge targets, tagged "from Document B". **Remove Document B** detaches it again — any of its styles you never actually merged into are removed with it.
...
11. Click **Save your file** (New Styles panel footer) at any time to download the result. The file keeps its original format and filename, with `-MASHED` appended (e.g. `Report.docx` → `Report-MASHED.docx`).
