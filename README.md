...
7. Have a second, already-clean Word file with the styles you want to standardize on? Use **Upload your own** (very bottom of the New Styles panel, beside **Clear list**) to import its style definitions as additional merge targets, tagged "from Document B". **Remove Document B** detaches it again — any of its styles you never actually merged into are removed with it.
8. **Undo** (top of the New Styles panel's footer, left of "Mash it") reverts the most recent merge or **Clear list** (the footer's very bottom row), one step at a time.
...
10. First time here? A short guided tour (five steps, skippable and rewindable) runs on your first visit. Click **Walkthrough** (top right) at any time to replay it, or **About** (next to it) for a written overview. (Adding `?tour` to the URL also forces it to run, handy for demos.)
11. Click **Save your file** (New Styles panel footer, just below Undo and Mash it) at any time to download the result. The file keeps its original format and filename, with `-MASHED` appended (e.g. `Report.docx` → `Report-MASHED.docx`).
