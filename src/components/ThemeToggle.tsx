...
/** Light/dark switch, in the app's violet palette. All
...
export function ThemeToggle({ isDark, onToggle }: ThemeToggleProps) {
  return (
    <button
      ...
      onClick={onToggle}
      className="theme-toggle relative h-7 w-14 shrink-0 cursor-pointer overflow-hidden rounded-full border border-chrome-edge focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300 focus-visible:ring-offset-2 focus-visible:ring-offset-chrome"
    >
      {/* Track: mid slate underneath, deep slate fading in on top (day = dark-ish, night = darker). */}
      <span className="absolute inset-0 bg-linear-to-r from-violet-800 to-violet-700" aria-hidden="true" />
      <span
        className={`absolute inset-0 bg-linear-to-r from-black to-violet-950 transition-opacity duration-500 ${
      ...
      <span
        className={`absolute right-1.5 top-3.5 h-2 w-4 rounded-full bg-violet-400 transition-all duration-500 ${
          isDark ? 'translate-x-3 opacity-0' : 'opacity-100'
        }`}
        aria-hidden="true"
      />
      <span
        className={`absolute right-4 top-1.5 h-1.5 w-3 rounded-full bg-violet-400/80 transition-all delay-75 duration-500 ${
          isDark ? 'translate-x-3 opacity-0' : 'opacity-100'
        }`}
        aria-hidden="true"
      />
      ...
      <span
        className={`absolute left-0.5 top-0.5 flex h-6 w-6 items-center justify-center rounded-full shadow-md transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
          isDark ? 'translate-x-7 bg-violet-100' : 'translate-x-0 bg-white'
        }`}
        aria-hidden="true"
      >
        <FontAwesomeIcon
          icon={faSun}
          className={`absolute h-3.5 w-3.5 text-violet-700 transition-all duration-500 ${
            isDark ? 'rotate-90 scale-50 opacity-0' : 'rotate-0 scale-100 opacity-100'
          }`}
        />
        <FontAwesomeIcon
          icon={faMoon}
          className={`absolute h-3 w-3 text-violet-800 transition-all duration-500 ${
